...
윈도우 설치 옆에 칼리 리눅스를 설치하면 여러 장점이 있어요. 하지만 설정 과정에서 주의가 필요해요. 먼저 Windows 설치에 중요한 데이터를 백업했는지 확인하세요. 하드 드라이브를 수정할 것이기 때문에 이 백업은 외부 매체에 저장하는 것이 좋아요. 백업을 완료한 후에는 기본적인 칼리 리눅스 설치 과정을 설명하는 [칼리 리눅스 하드 디스크 설치 가이드](/docs/installation/hard-disk-install/)를 살펴보는 것을 권장해요.

이 예시에서는 현재 컴퓨터의 디스크 공간을 100% 차지하는 윈도우(10) 설치와 함께 칼리 리눅스를 설치해 볼 거예요. 현재 윈도우 파티션의 크기를 줄여서 공간을 확보한 다음, 새로 만들어진 빈 파티션에 칼리 리눅스를 설치할 거예요.
