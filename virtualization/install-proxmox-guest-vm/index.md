...
- ```find / -name {파일이름}``` 명령으로 업로드된 파일을 찾으세요(예: ```find / -name kali-linux-2026.2-qemu-amd64.iso```)
- 파일이 있는 디렉토리로 이동하세요
- 파일 확장자를 *.iso에서 *.qcow2로 변경하세요(예: ```mv kali-linux-2026.2-qemu-amd64.iso kali-linux-2026.2-qemu-amd64.qcow2```)
- ```qm importdisk VM-ID {파일이름.qcow2} {저장소}``` 명령을 실행하세요(예: ```qm importdisk 108 kali-linux-2026.2-qemu-amd64.qcow2 local-lvm```)
- 셸을 닫으세요
