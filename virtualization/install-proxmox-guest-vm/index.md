...
## 미리 빌드된 가상 머신으로 Proxmox VM에 칼리 설치하기
...
- 파일 확장자를 *.iso로 변경하세요(즉, qcow2에서 iso로 바꿔요). 파일 이름을 기억해두세요(예: ```kali-linux-2025.4c-qemu-amd64.iso```)
- Proxmox를 열어주세요
...
- ```find / -name {파일이름}``` 명령으로 업로드된 파일을 찾으세요(예: ```find / -name kali-linux-2025.4c-qemu-amd64.iso```)
- 파일이 있는 디렉토리로 이동하세요
- 파일 확장자를 *.iso에서 *.qcow2로 변경하세요(예: ```mv kali-linux-2025.4c-qemu-amd64.iso kali-linux-2025.4c-qemu-amd64.qcow2```)
- ```qm importdisk VM-ID {파일이름.qcow2} {저장소}``` 명령을 실행하세요(예: ```qm importdisk 108 kali-linux-2025.4c-qemu-amd64.qcow2 local-lvm```)
- 셸을 닫으세요
...
- 이제 칼리를 시작할 준비가 됐어요. 로그인할 때 기본 계정 정보인 "kali/kali"를 사용하는 것 잊지 마세요(즉, 이 비밀번호는 나중에 꼭 변경하세요)

## Proxmox VM에 칼리 설치하기

단계별로 살펴보고 가능한 변경 사항도 알려드릴게요.

- [칼리 다운로드](/get-kali/#kali-installer-images)에서 최신 ISO를 다운로드하세요
...
- 이전에 업로드한 템플릿을 선택하세요
- 이 컨테이너의 사용 목적에 따라 더 많은 저장 공간을 할당할 수 있어요. 일반적인 칼리 설치의 경우 20GiB를 할당하는 것이 좋아요
- "코어" 수를 2로 늘리세요. 필요에 따라 더 적거나 많이 사용할 수 있어요
- 마찬가지로, 사용 목적에 따라 메모리 양을 늘릴 수 있어요. 일반적인 칼리 설치의 경우 2048MiB 메모리와 1024 스왑을 할당하는 것이 좋아요
