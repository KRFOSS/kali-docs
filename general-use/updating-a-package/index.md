...
칼리 리눅스에서 패키지를 업데이트하려면 칼리 리눅스가 기반으로 하는 데비안에서 제공하는 패키지 관리 도구를 사용할 수 있어요. 주요 패키지 관리 도구는 apt예요. 업데이트는 종종 취약점에 대한 패치를 포함하므로 시스템을 정기적으로 업데이트하는 것은 보안에 중요해요. 다음과 같은 방법으로 패키지를 업데이트할 수 있어요:
