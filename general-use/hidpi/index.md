...
이 과정을 더 쉽게 하기 위해, 칼리는 이제 HiDPI 모드를 제공해요. 이 모드는 GTK, QT 및 자바 기반 인터페이스에 대한 스케일링 팩터를 조정하여 사용자가 각각을 수동으로 수정할 필요가 없게 해요. 애플리케이션 메뉴에서 'Kali HiDPI mode'를 열거나 터미널에서 `kali-hidpi-mode`를 실행하여 이 모드를 전환할 수 있어요.
...
칼리에 로그인한 후, 배경 화면은 "정상"으로 보이지만 다른 모든 것이 "읽기에 조금 작게" 보일 수 있어요.
...
- 그래픽 방식:
  - 칼리 -> 설정 -> 모양새 -> **설정** -> **윈도우 스케일링**
  - 칼리 -> 설정 -> 모양새 -> **윈도우 관리자** -> **테마: `Kali-Dark-xHiDPI`**
