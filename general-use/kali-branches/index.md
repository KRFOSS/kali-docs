...
브랜치는 소프트웨어의 대체 버전으로, 이 경우에는 칼리 OS의 대체 버전이에요. 칼리 리눅스는 여러 브랜치를 가지고 있어 사용자가 패키지의 업데이트 상태를 어떻게 유지할지 결정할 수 있어요. 칼리 리눅스는 브랜치 사용을 포함하여 [여러 면에서 데비안과 유사](/docs/policy/kali-linux-relationship-with-debian/)해요.
...
- **kali-last-snapshot**은 사용자가 소프트웨어 제어에 대해 더 표준적인 느낌을 원할 때 사용할 수 있는 칼리의 브랜치예요. 새로운 릴리스마다 코드를 동결하고 `kali-rolling`을 `kali-last-snapshot`에 병합하는데, 이때 사용자는 [버전이 지정된 릴리스](/releases/) 간의 모든 업데이트를 받게 돼요(예: 2020.3 -> 2020.4). 패키지가 업데이트되지 않고(다음 릴리스까지 "포인트 릴리스"이므로) 릴리스 테스트를 거치기 때문에 더 안정적인 경우가 많아요. 이것이 "가장 안전한" 옵션이에요.

이 브랜치들 각각은 칼리 OS의 완전한 배포판을 제공해요. 둘 중 하나만 사용해야 하며, 둘 다 동시에 활성화하는 것은 의미가 없어요.
...
[데비안](https://www.debian.org/releases/)에는 세 가지 주요 옵션이 있어요:
...
**Stable**은 "안전한" 데비안 브랜치예요. 약 두 달마다 "[포인트 릴리스](https://wiki.debian.org/DebianReleases/PointReleases)"로 업데이트되는데, 이는 주로 보안 업데이트일 뿐이에요. 패키지는 잠재적인 비호환성과 불안정성 때문에 이 기간 동안 일반적으로 버전 업그레이드를 받지 않아요. 이것은 **kali-last-snapshot**의 데비안 동등물이에요.

**Testing**은 데비안 "롤링" 배포판과 가장 가까운 것으로, "롤링"은 패키지 업데이트가 사용 가능해지는 즉시 배포된다는 것을 의미해요. 칼리는 2016년 1월부터 이 브랜치를 **kali-rolling**의 시작점으로 사용하고 있어요.

**Unstable**은 데비안 패키지 개발 직후에 있어요. 패키지가 생성되었지만 완전히 테스트되지 않았어요. 칼리는 롤링 배포판이기 때문에 이에 해당하는 것이 없어요.

칼리가 데비안과 어떻게 관련되는지에 대한 자세한 내용은 해당 주제에 관한 [정책 페이지](/docs/policy/kali-linux-relationship-with-debian/)를 참조하세요.
...
**경고: kali-dev는 모든 칼리 미러에서 누구에게나 공개적으로 접근할 수 있지만, 이 배포판은 정기적으로 손상되기 때문에 최종 사용자가 사용해서는 안 돼요**.

이 저장소는 실제로 Debian의 Testing 배포판에 kali-dev-only 저장소에서 사용할 수 있는 모든 칼리 특화 패키지가 강제로 주입된 것이에요. 칼리 패키지는 Debian 패키지보다 우선시돼요. Testing이 변경될 때 일부 칼리 패키지를 업데이트해야 하는 경우가 있지만, 이것이 즉시 이루어지지 않을 수 있어요. 이 기간 동안 kali-dev가 손상될 가능성이 높아요. 이 저장소는 칼리 개발자가 업데이트된 패키지를 푸시하는 곳이며 kali-rolling을 만드는 데 사용되는 기반이에요.
