...
1. First, begin by imaging the latest Kali Linux ISO (currently [2021.4](/downloads/)) to your USB drive as described in [this article](/docs/usb/live-usb-install-with-windows/). We're going to assume that the two partitions created by the imaging are `/dev/sdb1` and `/dev/sdb2`. This can be verified with the command `fdisk -l`.
...
```console
kali@kali:~$ end=7GiB
kali@kali:~$ read start _ < <(du -bcm kali-linux-2021.4-live-amd64.iso | tail -1); echo $start
...
1. Image the latest Kali Linux ISO (currently 2021.4) to your USB drive as described in [this article](/docs/usb/live-usb-install-with-windows/).
