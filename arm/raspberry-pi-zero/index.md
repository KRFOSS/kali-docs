...
[라즈베리 파이 제로](https://www.raspberrypi.org/products/raspberry-pi-zero/)는 싱글 코어 1GHz 프로세서와 512MB RAM을 갖추고 있어요. [라즈베리 파이 제로 W](/docs/arm/raspberry-pi-zero/)와 달리, 라즈베리 파이 제로는 보드에 **네트워크 기능이 없어요**, 그래서 네트워킹을 위해서는 USB 어댑터를 사용해야 해요. 칼리 리눅스는 외장 마이크로 SD 카드에 설치돼요.
...
_dd_ 작업이 완료되면 마이크로 SD를 꽂은 상태로 라즈베리 파이 제로를 부팅하세요.
