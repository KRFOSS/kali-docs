...
- [개요](#개요)
- [우분투 호스트에서 명령줄 칼리 LXD 컨테이너](#Ubuntu-호스트에서-명령줄-Kali-LXD-컨테이너)
- [우분투 호스트에서 GUI 칼리 LXD 컨테이너](#Ubuntu-호스트에서-GUI-Kali-LXD-컨테이너)
- [칼리 호스트에서 권한 있는 칼리 LXC 컨테이너](#Kali-호스트에서-권한-있는-Kali-LXC-컨테이너)
- [칼리 호스트에서 권한 없는 칼리 LXC 컨테이너](#Kali-호스트에서-권한-없는-Kali-LXC-컨테이너)
- [참고 자료](#참고자료)
...
LXD는 차세대 시스템 컨테이너 관리자입니다. 가상 머신과 유사한 사용자 경험을 제공하지만 대신 리눅스 컨테이너를 사용합니다. 이것은 다양한 Linux 배포판에서 사용할 수 있는 사전 제작된 이미지를 기반으로 하며, 매우 강력하면서도 꽤 간단한 REST API를 중심으로 구축되었습니다.

<u>LXD vs LXC:</u>

LXD는 둘 중 더 편리하지만 우분투나 다른 배포판(칼리 등)에서는 스냅 패키지로만 사용할 수 있습니다.

LXC는 더 많은 배포판에서 사용 가능하며 기본적으로 지원되고 snapd 실행이 필요하지 않기 때문에 칼리에서 선호됩니다.

---

### 우분투 호스트에서 명령줄 칼리 LXD 컨테이너
