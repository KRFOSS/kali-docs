---
title: 칼리 WSL
description: 리눅스용 윈도우 하위 시스템 설정 준비하기
icon:
weight: 10
...
리눅스용 윈도우 하위 시스템(WSL)에는 [두 가지 버전](https://docs.microsoft.com/en-us/windows/wsl/compare-versions)인 "WSL 1"과 "WSL 2"가 있습니다. WSL 설치 시 **WSL 2가 권장**되며 기본 옵션입니다. 이는 WSL 2가 Hyper-V 가상 머신(WSL이 관리) 내에서 실제 리눅스 커널을 사용하기 때문입니다. WSL 1은 [WINE](https://www.winehq.org/)과 유사한 "번역 계층"을 사용합니다.

{{% notice info %}}
WSL 1에서의 "번역 계층"은 리눅스 시스템 콜(시스템 호출)을 윈도우 NT 커널이 이해할 수 있는 형태로 변환해주는 소프트웨어 레이어를 의미합니다. 이 계층은 리눅스 애플리케이션이 실행하려는 커널 수준 명령들을 가로채서 윈도우가 이해할 수 있는 동등한 명령으로 바꿔줍니다.
{{% /notice %}}

설치 과정이 더 간단한 윈도우 11을 사용하는 것이 좋지만, 윈도우 10을 사용해도 가능합니다. 다만 몇 가지 추가 단계가 필요합니다.
윈도우 10을 사용하는 경우 최대한 최신 업데이트를 적용해 두면 좋습니다. 특히 이 문서 작성 시점 기준으로 2022년 11월 패치가 적용되어 있으면 좋습니다.
...
WSL 2에서 칼리를 실행하기 위한 권장 방법은 다음과 같습니다:
...
2. 메시지가 표시되면 재부팅합니다.
3. 다음 링크에서 WSL 2 리눅스 커널을 다운로드하고 설치합니다. [aka.ms/wsl2kernel](https://aka.ms/wsl2kernel)
3. 재부팅합니다.
...
5. [마이크로소프트 스토어: 칼리 리눅스](https://apps.microsoft.com/store/detail/kali-linux/9PKR34TNCV07)에서 설치합니다
   - **참고**: 기존에 Kali WSL 1이 설치되어 있다면 `wsl --set-version kali-linux 2` 명령으로 업그레이드할 수 있습니다!
6. `kali` 명령어를 실행하여 새 사용자 생성 등 초기 설정을 완료합니다.

## 윈도우 버전

윈도우 11 사용자는 더 쉽게 [WSL 설치](#wsl-1-설치하기)로 바로 넘어갈 수 있습니다. 하지만 윈도우 10을 사용한다면 OS 빌드/버전을 확인해야 합니다. WSL의 최소 요구 사항은 다음과 같거나 그 이상이어야 합니다.
...
윈도우 10 `20H1`이 권장되는 이유는 WSL 헬퍼 스크립트가 이미 사전 설치되어 있어 명령줄에서 빠르게 설치할 수 있기 때문입니다.
또한 윈도우 10을 최신 버전으로 업데이트하면 WSL 헬퍼 스크립트가 Microsoft Store에서 업데이트를 가져올 수 있습니다.
윈도우 11이 윈도우 10보다 선호되는 이유는 아직 백포트되지 않은 내부 하드코딩된 URL 때문인데, 이는 곧 해결될 것으로 기대됩니다.
...
윈도우의 "옵션 기능"을 통해 WSL을 활성화하는 것은 필수 요구사항입니다. 하지만 필요에 따라 [마이크로소프트 스토어: 리눅스용 윈도우 하위 시스템](#wsl-in-microsoft-store)를 사용하여 WSL을 업데이트하면 기능 확장 및 버그 수정을 받을 수 있습니다.

WSL(WSL 1 또는 WSL 2)을 설치하는 방법은 여러 가지가 있습니다:

- [WSL 헬퍼 스크립트](#wsl-헬퍼-스크립트) (**가장 간단**)
  - _윈도우 10 `20H1` (빌드: `19041`, 버전: `2004`) 이상 필요_
  - WSL 2도 함께 설치됨
  - _2022년 11월 패치가 적용되면 더 나은 경험 제공_ <!-- due to it pulling updates from the Microsoft store to be the most up-to-date -->
- [Dism](#dism) (**권장**)
- [PowerShell](#powershell)

윈도우 버전과 패치 상태에 따라 설치 경험이 달라질 수 있습니다.

### WSL 헬퍼 스크립트

이 방법이 가장 쉽지만, 윈도우 10 `20H1` (빌드: `19041`, 버전: `2004`) 이상이 필요합니다. 그렇지 않으면 이 방법을 시도할 때 `'wsl'은(는) 내부 또는 외부 명령, 실행할 수 있는 프로그램, 또는 배치 파일이 아닙니다.`라는 오류가 발생합니다.
...
관리자 명령 프롬프트에서 다음 명령을 실행하면 WSL이 설치되지 않은 경우 WSL을 설정하고 [칼리 WSL을 설치](#kali-wsl-설치하기)합니다:
...
_2022년 11월 패치 이전의 윈도우 10에서는 알려진 문제로 인해 이 방법을 사용하면 오래된 버전의 칼리(칼리 2019.2)가 WSL에 설치될 수 있습니다. 이 경우 마이크로소프트 스토어에서 칼리를 [재설치](#install-kali-wsl)하는 것이 좋습니다. 윈도우 11을 포함한 최신 버전에서는 이 문제가 없습니다._
...
이제 [칼리 WSL을 설치](#kali-wsl-설치하기)하거나 [WSL 2로 업그레이드](#wsl-1에서-wsl-2로-수동-업그레이드)할 수 있습니다.
...
다음으로 WSL 리눅스 커널 업데이트를 수동으로 설치해야 합니다. 다음에서 다운로드할 수 있습니다:
...
이제 칼리를 설치하면 WSL 2 버전으로 설치됩니다.

**WSL 배포판 업그레이드**:

그러나 이전에 칼리 WSL 1을 설치한 경우 다음과 같이 업그레이드할 수 있습니다:
...
이 방법은 WSL을 [설치](#dism)하는 것이 아니라 업데이트하는 것이므로, 마이크로소프트의 리눅스용 윈도우 하위 시스템 옵션 기능이 활성화되어 있어야 합니다.
마이크로소프트 스토어를 사용하면 윈도우 업데이트 주기("패치 화요일") 외에도 [업데이트가 배포](https://devblogs.microsoft.com/commandline/a-preview-of-wsl-in-the-microsoft-store-is-now-available/)되므로 항상 최신 버전을 유지할 수 있습니다.
<!--
...
이 기능을 사용하려면 윈도우가 최신 상태(OS 업그레이드가 아닌 업데이트 패치 적용)인지 확인해야 합니다.
<!--
...
_윈도우 10 사용자는 2022년 11월 패치가 적용된 경우 `wsl --install` 명령이 마이크로소프트 스토어([WSL](#microsoft-store에서의-wsl)과 [칼리](#microsoft-store의-kali) 모두)에 대한 바로가기가 됩니다._


옵션 기능을 활성화한 후 [마이크로소프트 스토어](https://apps.microsoft.com/store/detail/windows-subsystem-for-linux/9P9TQF7MRM4R)로 이동하여 `리눅스용 윈도우 하위 시스템 (Windows Subsystem for Linux)`를 검색하고 "설치"를 클릭합니다:

![윈도우 10 WSL 스토어](wsl-store-10.png)
...
- [마이크로소프트 스토어](#microsoft-store의-kali) (**권장** & **가장 최신**)
...
그래픽 인터페이스가 있는 [마이크로소프트 스토어](https://apps.microsoft.com/store/detail/kali-linux/9PKR34TNCV07)를 사용하거나 PowerShell을 통해 직접 다운로드할 수 있습니다.

_윈도우 10 사용자는 2022년 11월 패치가 적용된 경우 `wsl --install` 명령이 마이크로소프트 스토어([WSL](#microsoft-store에서의-wsl)과 [칼리](#microsoft-store의-kali) 모두)에 대한 바로가기가 됩니다._

[마이크로소프트 스토어: 칼리 리눅스](https://apps.microsoft.com/store/detail/kali-linux/9PKR34TNCV07)로 이동하여 "설치"를 클릭합니다.
...
_2022년 11월 패치 이전의 윈도우 10에서는 알려진 문제로 인해 이 방법을 사용하면 오래된 버전의 칼리(칼리 2019.2)가 WSL에 설치될 수 있습니다. 마이크로소프트 스토어 방식을 사용하는 것이 좋습니다. 윈도우 11을 포함한 최신 버전에서는 이 문제가 없습니다._
<!-- https://github.com/microsoft/WSL/pull/8774 -->
