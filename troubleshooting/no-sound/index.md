...
칼리를 업그레이드하는 사용자라면, 평소처럼 시스템을 업그레이드하면 됩니다. 아래 두 명령어를 실행하세요:
...
이 문제는 실제 하드웨어에 칼리를 설치한 경우에만 해당돼요. 일부 최신 인텔 사운드카드는 작동하기 위해 펌웨어가 필요한데, 이 펌웨어가 시스템에 설치되지 않았을 수 있어요. 확실하지 않다면 설치해보세요. 아무런 해도 끼치지 않아요!:
