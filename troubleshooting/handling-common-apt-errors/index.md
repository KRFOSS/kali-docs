...
칼리 리눅스처럼 롤링 배포판(rolling distribution, 계속 업데이트되는 배포판)에서는 패키지와 의존성이 자주 바뀌기 때문에, 새 패키지를 설치하려면 기존 패키지를 지워야 할 때가 많아요.  
...
내 상황이 어떤 경우인지 알고 싶다면, 데비안 쪽 패키지 페이지를 찾아보거나 칼리 패키지라면 [bugs.kali.org](https://bugs.kali.org/)에 버그 리포트가 있는지 확인해보세요. 어쨌든 대부분의 경우 1~2주 내에 해결되는 경우가 많아요.
